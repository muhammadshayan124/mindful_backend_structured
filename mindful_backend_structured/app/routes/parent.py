...
async def parent_children(user=Depends(supabase_user)):
    """
    Returns the parent's children (id + display_name).
    """
...
async def parent_overview(
    ...
    user=Depends(supabase_user),
):
    """
    Overview of last N days per child:
    - avg sentiment, high risk counts, wellbeing score
    - heuristic risk flag (low/medium/high)
    """
    parent_id = await ensure_parent(user_id_from(user))
    kids = await list_children_for_parent(parent_id)
    ...
    for k in kids:
        ...
        pts = list(reversed(pts))  # chronological
        ...
        result.append(
            ChildOverview(
                ...
                last_7_days=[
                    DayPoint(
                        day=p["day"],
                        avg_sentiment=p.get("avg_sentiment") or 0.0,
                        high_risk_count=p.get("high_risk_count") or 0,
                        wellbeing=int(p.get("wellbeing_score") or 0),
                    )
                    for p in pts
                ],
            )
...
async def parent_child_timeline(
    child_id: str,
    days: int = Query(7, ge=1, le=30),
    user=Depends(supabase_user),
):
    """
    Timeline events for a single child from multiple sources.
    NOTE: For production, verify child_id belongs to this parent before returning.
    """
    # You may add an ownership check here using ensure_parent + list_children_for_parent
    ...
    chats = await sb_get(
        "chat_messages",
        {
            "select": "created_at,role,content,sentiment,triage_level",
            "child_id": "eq." + child_id,
            "order": "created_at.desc",
            "limit": str(days * 20),
        },
    ...
    games = await sb_get(
        "game_events",
        {
            "select": "created_at,activity,delta",
            "child_id": "eq." + child_id,
            "order": "created_at.desc",
            "limit": str(days * 20),
        },
