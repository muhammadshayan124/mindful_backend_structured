...
async def create_link_code(parent_id: str, ttl_minutes: int = 1440) -> Dict[str, str]:
    ...
    await sb_post("family_links", [{"code": code, "parent_id": parent_id, "expires_at": iso(expires_at)}])
    return {"code": code, "expires_at": iso(expires_at)}

async def consume_link_code(code: str, child_id: str) -> Dict[str, Any]:
    link = await sb_get("family_links", {"select": "code,parent_id,expires_at,used_by_child", "code": "eq." + code})
    if not link: 
    ...
    await sb_patch("children", {"id": "eq." + child_id}, {"parent_id": lk["parent_id"]})
    await sb_patch("family_links", {"code": "eq." + code}, {"used_by_child": child_id})
    return {"parent_id": lk["parent_id"]}

async def list_children_for_parent(parent_id: str) -> List[Dict[str, Any]]:
    return await sb_get("children", {"select": "id,display_name", "parent_id": "eq." + parent_id})
