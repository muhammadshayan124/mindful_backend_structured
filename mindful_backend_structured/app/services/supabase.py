...
async def sb_get(table: str, params: Dict[str, str]) -> List[JSON]:
    async with httpx.AsyncClient(timeout=20.0) as c:
        r = await c.get(f"{settings.SUPABASE_URL}/rest/v1/{table}", headers={
...
async def sb_post(table: str, rows: List[JSON]):
    async with httpx.AsyncClient(timeout=20.0) as c:
        r = await c.post(f"{settings.SUPABASE_URL}/rest/v1/{table}", headers={
            "apikey": settings.SUPABASE_SERVICE_ROLE_KEY,
            "Authorization": f"Bearer {settings.SUPABASE_SERVICE_ROLE_KEY}",
...
async def sb_patch(table: str, match: Dict[str, str], patch: Dict[str, Any]):
    async with httpx.AsyncClient(timeout=20.0) as c:
        r = await c.patch(f"{settings.SUPABASE_URL}/rest/v1/{table}", headers={
            "apikey": settings.SUPABASE_SERVICE_ROLE_KEY,
            "Authorization": f"Bearer {settings.SUPABASE_SERVICE_ROLE_KEY}",
            "Content-Type": "application/json",
            "Prefer": "return=minimal"
